...
class ThorlabsUC480Class(PylablibInterfaceClass) :
    ...
    def __init__(self, cameraNumber, triggerMode=0, exposurems=1, 
        ...
        Return: 
            ThorlabsUC480Class camera object
        """
        ...
        if not( str(self.cameraConfig['serial']) in [info.serial_number for info in uc480.list_cameras()] )  :
        ...
        try : 
            self.pylablibCamera = uc480.UC480Camera(dev_id = uc480.find_by_serial(str(self.cameraConfig['serial'])))
        except :
...
class ExampleModelClass(ThorlabsUC480Class) :
    """Model-specific class for ... Thorlabs UC480 (IDS) camera using pylablib python driver.
    
        Child Class of ThorlabsUC480Class. 
