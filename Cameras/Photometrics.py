...
try : 
    ...
    pylablibPVCAMDriverInstalled = True
except :
    print('ERROR ! : Tried to load pylablib Photmetrics driver, but it is not installed')
    pylablibPVCAMDriverInstalled = False

class PhotometricsClass(PylablibInterfaceClass) :
    ...
    def __init__(self, cameraNumber, triggerMode=0, exposurems=1, 
        ...
        Return: 
            PhotometricsClass camera object
        """
        ...
        if not(self.cameraDriverInstalled) or not(pylablibPVCAMDriverInstalled) :
            self.cameraDriverInstalled = False
            return 
        # check if camera is here 
        if not( str(self.cameraConfig['serial']) in  Photometrics.list_cameras() ) :
            print('ERROR ! : Could not find camera '+str(cameraNumber)+' with its serial number used as name among connected cameras')
            return 
        # here is the main camera object that will be used to access camera via driver
        try : 
            self.pylablibCamera = Photometrics.PvcamCamera(str(self.cameraConfig['serial']))
        except :
        ...
        self.imageBitDepth = int(self.pylablibCamera.cav["BIT_DEPTH"])
        if self.imageBitDepth != int(self.cameraConfig['imageBitDepth']) :
            print('Warning ! : Overwrite Camera.imageBitDepth with the one abtained from Photometrics camera S/N '+str(self.cameraConfig['serial']))
    ...
    def setTriggerMode(self, triggerMode):
        ...
        if triggerMode==0:#'external'
            self.pylablibCamera.set_trigger_mode('e_rise_edge')
        elif triggerMode==1:#'software'
            self.pylablibCamera.set_trigger_mode('e_soft_edge')
    ...
    def setGaindB(self, gaindB) :
        ...
        WARNING : Not Defined for PhotometricsClass camera . 
        
        Args:
            gaindB (float) : hardware gain of the camera in dB.
        """
        print("Warning  : setGaindB Not Defined for PhotometricsClass camera.")
        self.gaindB = 0
...
class ExampleModelClass(PhotometricsClass) :
    """Model-specific class for ... Photometrics camera using pylablib python driver.
    
        Child Class of PhotometricsClass. 
