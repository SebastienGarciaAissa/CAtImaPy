...
as an interface and parent for all maufacturer/model specific camera classes.

The configurations of cameras are set in the *Cameras/Config.py* file as a list ``camerasConfigs`` of dictionnaries.

The creation of the ``Camera`` object, abttribute of ``mainWin``, is realized by the factory/builder function :func:`create_Camera`.
...
Idealy, the creator should load a child class for the specified model with proper initialization of this model. 
...
Either directly by connecting a python driver, for example, :class:`FLIRPySpinClass` connectected the PySpin driver package provided by FLIR.
