...
The remaing few variable connections go through dedicated ``mainWin`` methods with format ``on_<variable name>_valueChanged()`` or ``on_<variable name>_stateChanged()``.
