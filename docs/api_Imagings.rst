...
This class defines the ``Imaging`` object, abttribute of ``mainWin``. 
...
Fitting of functions performed in :class:`ImagingClass` methods use the sub-module ``fittool``. 
...
The defined functions available for fitting are listed in ``fitFunctionsList``. 
